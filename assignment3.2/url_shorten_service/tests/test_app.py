...
class TestApi(unittest.TestCase):
    ...
    # base_url = "http://145.100.130.38:32414"
    # auth_url = "http://145.100.130.38:31727"
    # base_url = "http://145.100.130.39:32414"
    # auth_url = "http://145.100.130.39:31727"
    base_url = "http://145.100.130.39.nip.io:31570/shorten"
    auth_url = "http://145.100.130.39.nip.io:31570/auth"
    ...
    def populate_variables_from_csv(self):
        with open('read_from.csv', 'r', encoding='utf-8') as f:
            reader = csv.reader(f)
            next(reader, None)
    ...
    def test_get_request_with_id_expect_301(self):
        ...
        self.assertEqual(response.json().get("value"), expected_value,
                         "Expected response body to be " + expected_value + " , but got " + response.json().get(
                             "value"))
    ...
    def test_deletion_all(self):
        ...
        # make a get all to check that stuff is erased
        endpoint = "/"
        url = f"{self.base_url}{endpoint}"
        response = requests.get(url, headers=self.headers)
